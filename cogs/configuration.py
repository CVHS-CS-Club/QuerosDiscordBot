...
class Configuration(commands.Cog):
	...
	async def curse_filter(self, ctx, level: str):
		"""Profanity filter. 4 options:
		off = no filter
		low = no slurs
		med = no harsh cursing (ass and mild language is allowed)
		strict = no bad language of any kind."""
		if level not in ('off', 'low', 'med', 'strict'):
			await ctx.send("Please choose a valid level")
			return
		server = ctx.message.guild.id
		try:
			configcol.update_one({"$and": [{"guild": server}, {"cfg_type": 'profanity'}]}, {"$set":{'cfg_type':'profanity', 'guild':server, 'level':level}}, upsert=True)
		except Exception as e:
			print(e)
		await ctx.send("Profanity filter level set to: **" + level + "**")
